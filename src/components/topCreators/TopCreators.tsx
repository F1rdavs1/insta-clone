...
const TopCreators: React.FC = () => {
  const { data: users = [], isLoading } = useGetAllUsersQuery(true);
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-2 users-wrapper gap-4">
        {isLoading
          ? Array.from({ length: 8 }).map((_, index) => (
              <div
                key={index}
                className="topcreator-wrapper flex flex-col items-center py-[24px] px-[4px] border border-[#1F1F22] rounded-[20px] w-[150px]"
              >
                <div className="w-[54px] h-[54px] bg-gray-500 rounded-full mb-2 animate-pulse" />
                <div className="bg-gray-500 w-[70px] h-[16px] rounded mt-2 animate-pulse" />
                <div className="bg-gray-500 w-[90px] h-[24px] rounded mt-2 animate-pulse" />
              </div>
            ))
